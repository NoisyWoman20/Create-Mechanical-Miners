...
public class BrassStressMinerVisual extends SingleAxisRotatingVisual<BrassStressMinerBlockEntity> {
	private int retryDelay = 0;
	private final boolean isUpper;

	public BrassStressMinerVisual(VisualizationContext context, BrassStressMinerBlockEntity blockEntity, float partialTick) {
		super(context, blockEntity, partialTick, Models.partial(getModel(blockEntity)));
		isUpper = (blockEntity.getBlockState().getValue(BrassStressMinerBlock.HALF) == DoubleBlockHalf.UPPER);
	}

	@Contract(pure = true)
	private static PartialModel getModel(@NotNull BrassStressMinerBlockEntity be) {
		PartialModel model;

		if (be.getBlockState().getValue(BrassStressMinerBlock.HALF) == DoubleBlockHalf.UPPER) {
			model = CMMPartialModels.BRASS_STRESS_MINER_HEAD_UPPER;
		} else {
	...
	public void tick(Context context) {
		...
		if (!isUpper && this.blockEntity.getLevel() != null && ++retryDelay >= 5) {
			...
			if (above instanceof BrassStressMinerBlockEntity upper) {
